...
import com.bakkle.bakkle.AddItem.MaterialCamera.TimeLimitReachedException;
...
public abstract class BaseCaptureActivity extends AppCompatActivity implements BaseCaptureInterface
{
    ...
    protected final void onSaveInstanceState(Bundle outState)
    {
        ...
        outState.putLong(com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.LENGTH_LIMIT,
                mLengthLimit);
    ...
    protected final void onCreate(Bundle savedInstanceState)
    {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            final int primaryColor = getIntent().getIntExtra(
        ...
        if (null == savedInstanceState) {
            checkPermissions();
            mLengthLimit = getIntent().getLongExtra(
                    com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.LENGTH_LIMIT,
                    -1);
        } else {
            ...
            mLengthLimit = savedInstanceState.getLong(
                    com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.LENGTH_LIMIT,
                    -1);
    ...
    private void checkPermissions()
    {
        ...
        final boolean videoGranted = ContextCompat.checkSelfPermission(this,
                Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED;
        if (videoGranted) {
            showInitialRecorder();
        } else {
            final boolean audioGranted = ContextCompat.checkSelfPermission(this,
    ...
    public final void onBackPressed()
    {
        ...
        if (frag != null && frag instanceof com.bakkle.bakkle.AddItem.MaterialCamera.internal.PlaybackVideoFragment && allowRetry()) {
    ...
    public boolean countdownImmediately()
    {
        return getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.COUNTDOWN_IMMEDIATELY,
                false);
    }
    ...
    public final void onRetry(@Nullable String outputUri)
    {
        ...
        if (getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.RETRY_EXITS,
                false)) {
            setResult(RESULT_OK, new Intent().putExtra(MaterialCamera.STATUS_EXTRA,
                    MaterialCamera.STATUS_RETRY));
            finish();
            return;
        }
        getFragmentManager().beginTransaction().replace(R.id.container, createFragment()).commit();
    ...
    public final void onShowPreview(@Nullable final String outputUri, boolean countdownIsAtZero)
    {
        if ((shouldAutoSubmit() && (countdownIsAtZero || !allowRetry() || !hasLengthLimit())) || outputUri == null) {
            ...
            Fragment frag = com.bakkle.bakkle.AddItem.MaterialCamera.internal.PlaybackVideoFragment.newInstance(
                    outputUri, allowRetry(), getIntent().getIntExtra(
                            com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.PRIMARY_COLOR,
                            0));
    ...
    {
        return getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.ALLOW_RETRY,
                true);
    }
    ...
    {
        return getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.AUTO_SUBMIT,
                false);
    }
    ...
    {
        if (uri != null) {
            setResult(Activity.RESULT_OK, getIntent().putExtra(MaterialCamera.STATUS_EXTRA,
                    MaterialCamera.STATUS_RECORDED).setDataAndType(Uri.parse(uri), "video/mp4"));
        }
        finish();
    }

    public final void useImage(String uri)
    {
        if (uri != null) {
            setResult(Activity.RESULT_OK, getIntent().putExtra(MaterialCamera.STATUS_EXTRA,
                    MaterialCamera.STATUS_PICTURE_TAKEN).setDataAndType(Uri.parse(uri), "image/jpg"));
        }
        finish();
    }
    ...
    {
        return getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.RESTART_TIMER_ON_RETRY,
                false);
    }
    ...
    {
        return getIntent().getBooleanExtra(
                com.bakkle.bakkle.AddItem.MaterialCamera.internal.CameraIntentKey.CONTINUE_TIMER_IN_PLAYBACK,
                false);
    }
}