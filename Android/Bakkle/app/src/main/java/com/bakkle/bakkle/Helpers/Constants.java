package com.bakkle.bakkle.Helpers;

/**
 * Created by vanshg on 9/9/2015.
 */
public class Constants
{
    public static String SHARED_PREFERENCES = "Bakkle";

    //SharedPreferences keys
    public static final String AUTH_TOKEN      = "auth_token";
    public static final String UUID            = "uuid";
    public static final String USER_ID         = "userID";
    public static final String LOCATION        = "locationString";
    public static final String LATITUDE        = "latitude";
    public static final String LONGITUDE       = "longitude";
    public static final String EMAIL           = "email";
    public static final String GENDER          = "gender";
    public static final String USERNAME        = "username";
    public static final String NAME            = "name";
    public static final String LOCALE          = "locale";
    public static final String FIRST_NAME      = "first_name";
    public static final String LAST_NAME       = "last_name";
    public static final String LOGGED_IN       = "LoggedIn";
    public static final String PRICE_FILTER    = "price_filter";
    public static final String DISTANCE_FILTER = "distance_filter";
    public static final String SEARCH_TEXT     = "search_text";
    public static final String NEW_USER        = "newuser";

    //Intent keys
    public static final String PICTURE_PATH     = "pic";
    public static final String VIDEO_PATH       = "vid";
    public static final String NUM_OF_PICS      = "num";
    public static final String TITLE            = "title";
    public static final String SELLER           = "seller";
    public static final String PRICE            = "price";
    public static final String DISTANCE         = "distance";
    public static final String SELLER_IMAGE_URL = "sellerImageUrl";
    public static final String BUYER_IMAGE_URL  = "buyerImageUrl";
    public static final String DESCRIPTION      = "description";
    public static final String PK               = "pk";
    public static final String PARENT           = "parent";
    public static final String MARK_WANT        = "markWant";
    public static final String GARAGE           = "garage";
    public static final String CHAT_ID          = "chatId";
    public static final String SELF_BUYER       = "self_buyer";
    public static final String IMAGE_URLS       = "urls";
    public static final String ITEM_ID          = "itemId";
    public static final String NUM_WANT         = "numWant";
    public static final String NUM_HOLD         = "numHold";
    public static final String NUM_MEH          = "numMeh";
    public static final String NUM_VIEW         = "numView";


    public static final String SHOW_TUORIAL     = "showTutorial";


}
