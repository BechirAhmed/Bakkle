...
public class PNLineChart: UIView{
        ...
        for linePoints: AnyObject in pathPoints {
            ...
            for var i:NSInteger = 0; i < (linePointsArray.count - 1); i += 1{
                ...
                if distance <= 10.0 {
                    ...
                    delegate?.userClickedOnLineKeyPoint!(touchPoint , lineIndex: pathPoints.indexOfObject(linePoints) ,pointIndex:(distance == distanceToP2 ? i + 1 : i) )
                }
            }
        }
    ...
    */
    ...
    public func strokeChart(){
        ...
        for var lineIndex = 0; lineIndex < chartData.count; lineIndex++ {
            ...
            if chartData.color != UIColor.blackColor() {
                ...
                chartLine.strokeColor =
                    PNGreenColor.CGColor
    ...
    func heightOfString(text: NSString!, width: CGFloat, font: UIFont) -> CGFloat{
        ...
        let tdic:NSDictionary = NSDictionary(objects: [font, NSFontAttributeName], forKeys: [])
        size = text.boundingRectWithSize(size, options: NSStringDrawingOptions.UsesFontLeading , attributes: tdic as? [String : AnyObject] as [String : AnyObject]?, context: nil).size
        ch = size.height
        return ch
