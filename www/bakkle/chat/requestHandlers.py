...
class ChatWSHandler(websocket.WebSocketHandler):
    ...
    def authenticate(self, request):
        ...
        try:
            device = Device.objects.filter(uuid=request['uuid']).filter(
    ...
    def getChats(self, request):
        try:
            ...
            chats = Chat.objects.filter(
                item__seller__pk=self.clientId).filter(item=item)
    ...
    def sendChatMessage(self, request):
        ...
        print("New message request: " + str(request))
        time1 = time.time()
        ...
        print("Notifying devices. Time elapsed: " +
              str((time.time() - time1) * 1000))
        ...
        print("Updating seller. Time elapsed: " +
              str((time.time() - time1) * 1000))

        if(chat.item.seller.pk in clients):
            for uuid in clients[chat.item.seller.pk]:
                clients[chat.item.seller.pk][uuid].write_message(
                    {'success': 1, 'messageOrigin': self.clientId, 'notificationType': 'newMessage', 'message': newMessage.toDictionary()})

        print("Updating buyer. Time elapsed: " +
              str((time.time() - time1) * 1000))
        ...
        print("Message sending complete. Time elapsed: " +
              str((time.time() - time1) * 1000))
        return {'success': 1}

    def getMessagesForChat(self, request):
        try:
            chat = Chat.objects.filter(Q(pk=request['chatId'])).filter(
                Q(buyer__pk=self.clientId) | Q(item__seller__pk=self.clientId))
